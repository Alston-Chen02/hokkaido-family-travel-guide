...
const drugstoreGuides={
  ...
  3:{intro:'白天在小樽堺町可順路買；晚上狸小路建議依所在丁目選一間，不必兩端來回跑。',shops:[
  ...
  4:{intro:'北廣島 OUTLET 館內就有藥妝店，午餐與購物都可留在館內，不必另外搭車。',shops:[
  ...
  5:{intro:'金森倉庫旁有一間可順路補買；返程日時間緊，僅在領隊明確放行且不影響機場報到時前往。',shops:[
...
const renderDrugstores=d=>{
  ...
  if(!guide)return '';
  return `<details class="drugstore-guide"><summary><span>💊 自由時間・附近藥妝</span><small>${guide.shops.length} 間可選</small></summary><div class="drugstore-body"><p class="drugstore-intro">${guide.intro}</p><div class="drugstore-list">${guide.shops.map(shop=>`<article class="drugstore-card"><span class="drugstore-zone">${shop.zone}</span><h4>${shop.name}</h4><p><strong>位置：</strong>${shop.place}</p><p><strong>通常營業：</strong>${shop.hours}</p><p>${shop.tip}</p><div class="drugstore-links"><a href="https://www.google.com/maps/search/?api=1&amp;query=${encodeURIComponent(shop.maps)}&amp;hl=zh-TW" target="_blank" rel="noopener noreferrer">Google 地圖（繁中）↗</a><a href="${shop.official}" target="_blank" rel="noopener noreferrer">官方店舖資訊${shop.lang?`（${shop.lang}）`:''} ↗</a></div></article>`).join('')}</div><p class="drugstore-caveat">Google 地圖與官方頁面已核對分店位置；營業時間、庫存及免稅條件可能調整，出發前或到店當日再查看。先拍集合點、設定回程提醒；購買藥品請向藥師確認適用性。</p></div></details>`;
};
...
if('serviceWorker' in navigator){
  ...
  navigator.serviceWorker.addEventListener('controllerchange',()=>{if(refreshing)return;refreshing=true;if(!sessionStorage.getItem('pwa-v19-reloaded')){sessionStorage.setItem('pwa-v19-reloaded','1');location.reload()}});
